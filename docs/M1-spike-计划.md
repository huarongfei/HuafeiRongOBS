...
## UI 改造：嵌入式控制台（M4 方向，替代临时 HFR(M1) 演示菜单）
用户反馈：临时演示菜单/散窗导致"操作感分裂" → 要求彻底嵌入并改 UI。
选定形态（与用户确认）：右侧**输出矩阵**为主 + 系统自动检测外接屏并**编号**、用户选编号即可投影；直接做**完整导播控制台布局**。
实施状态：
- S1（已编译零错误）：右侧停靠坞"输出矩阵（画布）"——显示器列表(#编号/名/分辨率/[主])、画布列表(★主画布+用户画布)、
  按钮：刷新屏/画布、+新建画布(1280x720)、删除画布、▶投影到选中屏(全屏受管)、■关闭所有投影；退出 aboutToQuit 统一关投影。
- S2（下一步）：每画布行内实时小预览（OBSQTDisplay 缩略）；"点哪操作哪"：选中画布后场景坞/切换台作用域跟随该画布。
